  ...
  <view class="detail">
    ...
    <view class="footer">
      <view class="activity-btn">
        ...
        <button type="primary" v-else-if="data0.isApplication.length === 0 && data0.isClose === 1" class="btn"
          :class="{ 'disabled-btn': isFull }" :disabled="isFull" @click="applyon">
          报名
        </button>
...
const getDetails = async (url: string) => {
  ...
  // 计算属性判断是否已满
  isFull.value = computed(() => data0.value.hbNum >= data0.value.hot).value
}
