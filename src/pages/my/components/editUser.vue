...
import { getPersonalInf, changePersonalInf, postUnbind } from '@/services/personalInf'
// type userInfo = {
//   name: string
//   studentId: string
//   img?: string
...
// 封装请求退出绑定函数
const packageUnbind = async () => {
  const data = {
    wxToken: uni.getStorageSync('wxToken'),
  }
  const res = await postUnbind(data)
  if (res.code === 200) {
    uni.showToast({
      title: '退出绑定成功',
      icon: 'success',
...
const unbind = () => {
  const wxToken = uni.getStorageSync('wxToken')
  if (!wxToken) {
    uni.showToast({
      title: '请通过微信快捷登录来退出绑定',
      icon: 'none',
      duration: 1000,
    })
    return
